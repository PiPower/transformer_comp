...
def train_with_adam(train_dataset, eng_word_count, pt_word_count ):
  num_layers = 4
  d_model = 128
  dff = 512
  num_heads = 8
  ...
  transformer_model.compile("Adam", loss=auto_reg_loss, metrics=[accuracy_function])
  history = transformer_model.fit(train_dataset, epochs=10)


train_pt, train_eng = load_texts("../datasets/eng-pt/train.txt", 10000)
...
train_dataset = preprocess_data(train_eng,train_pt, tokenizer_eng, tokenizer_pt)

eng_word_count = len(tokenizer_eng.word_index)+1
pt_word_count = len(tokenizer_eng.word_index)+1
train_with_adam(train_dataset, eng_word_count, pt_word_count)
